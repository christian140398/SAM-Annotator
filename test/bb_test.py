...
Examples:
  ...
  python test/bb_test.py 00004 --format voc
    ...
    if format_type == "auto":
        ...
        if format_type == "voc":
            annotation_path = os.path.join(
                args.output_dir, "bb_labels", f"{base_name}.xml"
            )
        elif format_type == "coco":
            annotation_path = os.path.join(
                args.output_dir, "bb_labels", f"{base_name}.json"
            )
    ...
    print(
        f"Loading bounding boxes from: {annotation_path} (format: {format_type.upper()})"
    )
    try:
        if format_type == "voc":
