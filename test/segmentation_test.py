...
from segmentation.sam_utils import rle_to_mask  # noqa: E402
        ...
        if seg:
            ...
            if isinstance(seg, dict) and "size" in seg and "counts" in seg:
                try:
                    ...
                    print(
                        f"Warning: Could not convert RLE to polygon for annotation {ann.get('id')}: {e}"
                    )
...
def draw_segmentations_opencv(
    image, segmentations, output_path=None, color_palette=None
):
    """
    ...
    for label_name, polygon_points, bbox in segmentations:
        ...
        if polygon_points is not None:
            ...
            cv2.polylines(
                img, [polygon_points], isClosed=True, color=color, thickness=2
            )
        ...
        (text_width, text_height), _ = cv2.getTextSize(
            label_name, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2
        )
        cv2.rectangle(
            img,
            (int(x), int(y) - text_height - 8),
            (int(x) + text_width + 4, int(y)),
            color,
            -1,
        )
        cv2.putText(
...
def draw_segmentations_matplotlib(
    image, segmentations, output_path=None, color_palette=None
):
    """
    ...
    for label_name, polygon_points, bbox in segmentations:
        ...
        color_rgb = (
            color_rgb_int[0] / 255.0,
            color_rgb_int[1] / 255.0,
            color_rgb_int[2] / 255.0,
        )
...
Examples:
  ...
  python test/segmentation_test.py 00004 --format voc
    ...
    if format_type == "auto":
        ...
        if format_type == "voc":
            annotation_path = os.path.join(
                args.output_dir, "segment_labels", f"{base_name}.xml"
            )
        elif format_type == "coco":
            annotation_path = os.path.join(
                args.output_dir, "segment_labels", f"{base_name}.json"
            )
    ...
    print(
        f"Loading segmentations from: {annotation_path} (format: {format_type.upper()})"
    )
    try:
        if format_type == "voc":
        ...
        for i, (label, points, bbox) in enumerate(segmentations, 1):
            polygon_info = (
                f"{len(points)} points" if points is not None else "no polygon"
            )
    ...
    color_palette = load_color_palette(LABEL_FILE)
    print(
        f"Loaded {len([k for k in color_palette.keys() if k != 'default'])} labels with colors"
    )
    ...
    if args.backend == "opencv":
        ...
        if not HAS_MATPLOTLIB:
            ...
            draw_segmentations_matplotlib(
                image, segmentations, args.output, color_palette
            )
