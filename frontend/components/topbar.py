...
from PySide6.QtWidgets import (
    ...
    QDialog, QDialogButtonBox, QLineEdit, QColorDialog, QFrame
)
from PySide6.QtCore import Qt, Signal, QSize
from PySide6.QtGui import QColor, QPainter, QIcon, QPixmap
from PySide6.QtWidgets import QStyledItemDelegate
from frontend.theme import get_topbar_style, get_button_style, get_combo_box_style, TOPBAR_BORDER
...
class TopBar(QWidget):
    ...
    def set_labels(self, labels: List[dict]):
        ...
        """
        self.labels = [LabelColorDelegate(l['id'], l['name'], l['color']) for l in labels]
    ...
    def update_label_combo(self):
        ...
        if self.selected_label_id:
            index = next((i for i, label in enumerate(self.labels) if label.id == self.selected_label_id), -1)
            if index >= 0:
    ...
    def set_selected_label(self, label_id: Optional[str]):
        ...
        self.selected_label_id = label_id
        if label_id:
            index = next((i for i, label in enumerate(self.labels) if label.id == label_id), -1)
            if index >= 0:
                self.label_combo.blockSignals(True)
                self.label_combo.setCurrentIndex(index)
                self.label_combo.blockSignals(False)
